...
const FACULTY_SEED_VERSION_KEY = "turotugma_faculty_seed_version";
const FACULTY_SEED_VERSION = "agreed_68_v1";
...
const DEFAULT_FACULTY_SEED = [
  {
    ...
    contact: "+639171234001",
    degree: "BSEd",
    major: "Mathematics",
    minor: "Science",
    ancillaryAssignments: ["Sports Development Office Adviser"],
    unavailablePeriods: ["Monday P1", "Friday P3"],
    subjectExpertise: ["MATH-JHS — Mathematics", "STEM-MATH — General Mathematics"],
    gradeLevelAssignments: ["Grade 7", "Grade 8", "Grade 9"],
  },
  {
    name: "John dela Cruz",
    sex: "Male",
    ...
    contact: "+639171234002",
    degree: "BSEd",
    major: "English",
    minor: "Filipino",
    ancillaryAssignments: ["Campus Journalism Coach"],
    unavailablePeriods: ["Tuesday P2"],
    subjectExpertise: ["ENG-JHS — English", "ORALCOMM — Oral Communication"],
    gradeLevelAssignments: ["Grade 7", "Grade 10", "Grade 11"],
  },
  {
    name: "Ana Reyes",
    sex: "Female",
    ...
    contact: "+639171234003",
    degree: "BSEd",
    major: "Science",
    minor: "Mathematics",
    ancillaryAssignments: ["Science Club Moderator"],
    unavailablePeriods: ["Wednesday P3"],
    subjectExpertise: ["SCI-JHS — Science", "BIO-SHS — Biology"],
    gradeLevelAssignments: ["Grade 8", "Grade 9", "Grade 12"],
  },
  {
    name: "Paul Mendoza",
    sex: "Male",
    ...
    employmentStatus: "Regular",
    email: "paul.mendoza@school.edu.ph",
    contact: "+639171234004",
    degree: "MAEd",
    major: "Filipino",
    minor: "None",
    ancillaryAssignments: ["Language Coordinator"],
    unavailablePeriods: ["Thursday P1", "Thursday P2"],
    subjectExpertise: ["FIL-JHS — Filipino", "KOMFIL — Komunikasyon at Pananaliksik"],
    gradeLevelAssignments: ["Grade 7", "Grade 11"],
  },
  {
    name: "Leah Garcia",
    sex: "Female",
    ...
    position: "Teacher III",
    employmentStatus: "Regular",
    email: "leah.garcia@school.edu.ph",
    contact: "+639171234005",
    degree: "BSEd",
    major: "Araling Panlipunan",
    minor: "English",
    ancillaryAssignments: ["DRRM Coordinator"],
    unavailablePeriods: ["Monday P3"],
    subjectExpertise: ["ARPAN — Araling Panlipunan", "UCSP — Understanding Culture Society and Politics"],
    gradeLevelAssignments: ["Grade 8", "Grade 9", "Grade 11"],
  },
  {
    ...
    email: "mark.villanueva@school.edu.ph",
    contact: "+639171234006",
    degree: "BSEd",
    major: "TLE",
    minor: "ICT",
    ancillaryAssignments: ["ICT Laboratory In-Charge"],
    unavailablePeriods: ["Tuesday P1", "Friday P2"],
    subjectExpertise: ["TLE-JHS — TLE", "ICT-SHS — Empowerment Technologies"],
    gradeLevelAssignments: ["Grade 9", "Grade 10", "Grade 12"],
  },
  {
    name: "Rosa Lim",
    sex: "Female",
    ...
    position: "Teacher I",
    employmentStatus: "Regular",
    email: "rosa.lim@school.edu.ph",
    contact: "+639171234007",
    degree: "BSEd",
    major: "MAPEH",
    minor: "Values Education",
    ancillaryAssignments: ["SPA Program Coordinator"],
    unavailablePeriods: ["Wednesday P1"],
    subjectExpertise: ["MAPEH — Music, Arts, PE & Health", "SPA — Special Program in the Arts"],
    gradeLevelAssignments: ["Grade 7", "Grade 8", "Grade 10"],
  },
  {
    name: "Kevin Torres",
    sex: "Male",
    ...
    position: "Teacher III",
    employmentStatus: "Regular",
    email: "kevin.torres@school.edu.ph",
    contact: "+639171234008",
    degree: "BSEd",
    major: "ABM",
    minor: "Mathematics",
    ancillaryAssignments: ["Entrepreneurship Coordinator"],
    unavailablePeriods: ["Thursday P3"],
    subjectExpertise: ["BUSFIN — Business Finance", "FABM — Fundamentals of ABM"],
    gradeLevelAssignments: ["Grade 11", "Grade 12"],
  },
];
...
function getInitialFaculty() {
  ...
  try {
    const appliedSeedVersion = window.localStorage.getItem(FACULTY_SEED_VERSION_KEY);
    if (appliedSeedVersion !== FACULTY_SEED_VERSION) {
      const seeded = buildAgreedFacultySeed();
      window.localStorage.setItem(FACULTY_STORAGE_KEY, JSON.stringify(seeded));
      window.localStorage.setItem(FACULTY_SEED_VERSION_KEY, FACULTY_SEED_VERSION);
      return seeded;
    }
...
function buildAgreedFacultySeed() {
  const totalTeachers = 68;
  const shsTeachers = 11;
  const jhsTeachers = totalTeachers - shsTeachers;
  const schoolIdPrefix = "300627";
  const createdSubjects = getCreatedSubjectCatalog();
  const jhsSubjects = getSubjectOptionsFromCatalog(createdSubjects, "jhs");
  const shsSubjects = getSubjectOptionsFromCatalog(createdSubjects, "shs");
  const coverageSubjects = pickJhsCoverageSubjects(jhsSubjects);
  const jhsGrades = ["Grade 7", "Grade 8", "Grade 9", "Grade 10"];
  const designations = DEPED_DESIGNATION_OPTIONS;
  const firstNames = [
    "Maria",
    "John",
    "Ana",
  ...
  const seeded = Array.from({ length: totalTeachers }, (_, index) => {
    const idNumber = index + 1;
    const employeeId = `${schoolIdPrefix}-${String(idNumber).padStart(3, "0")}`;
    const firstName = firstNames[index % firstNames.length];
    const lastName = lastNames[Math.floor(index / firstNames.length) % lastNames.length];
    const isShs = index >= jhsTeachers;
    const gradeAssignment = isShs ? (index % 2 === 0 ? "Grade 11" : "Grade 12") : jhsGrades[index % jhsGrades.length];

    return {
      ...EMPTY_FORM,
      name: `${firstName} ${lastName}`,
      sex: index % 3 === 0 ? "Male" : "Female",
      employeeId,
      position: designations[index % designations.length],
      employmentStatus: index % 8 === 0 ? "Probationary" : index % 11 === 0 ? "Part-Time" : "Regular",
      email: `teacher.${employeeId.replace("-", "")}@school.edu.ph`.toLowerCase(),
      contact: `+639${String(100000000 + idNumber).slice(-9)}`,
      degree: index % 6 === 0 ? "MAEd" : "BSEd",
      major: ["Mathematics", "English", "Filipino", "Science", "Araling Panlipunan", "MAPEH", "TLE"][index % 7],
      minor: ["Science", "English", "Filipino", "None"][index % 4],
      ancillaryAssignments: [ANCILLARY_POOL[index % ANCILLARY_POOL.length]],
      unavailablePeriods: [FALLBACK_PERIOD_OPTIONS[index % FALLBACK_PERIOD_OPTIONS.length]],
      subjectExpertise: [],
      gradeLevelAssignments: [gradeAssignment],
      ancillaryInput: "",
    };
  });

  let cursor = 0;
  jhsGrades.forEach((grade) => {
    coverageSubjects.forEach((subject) => {
      for (let count = 0; count < 4; count += 1) {
        const teacherIndex = (cursor + count * 7) % jhsTeachers;
        addUniqueValue(seeded[teacherIndex].gradeLevelAssignments, grade);
        addUniqueValue(seeded[teacherIndex].subjectExpertise, subject);
      }
      cursor = (cursor + 5) % jhsTeachers;
    });
  });

  const fallbackJhsSubject = coverageSubjects[0] || jhsSubjects[0] || "";
  for (let index = 0; index < jhsTeachers; index += 1) {
    if (seeded[index].subjectExpertise.length === 0 && fallbackJhsSubject) {
      addUniqueValue(seeded[index].subjectExpertise, fallbackJhsSubject);
    }

    if (seeded[index].gradeLevelAssignments.length === 0) {
      addUniqueValue(seeded[index].gradeLevelAssignments, jhsGrades[index % jhsGrades.length]);
    }
  }

  const shsIndices = Array.from({ length: shsTeachers }, (_, offset) => jhsTeachers + offset);
  const shsExpertiseSubjects = shsSubjects.slice(0, 3);
  shsIndices.forEach((index, offset) => {
    seeded[index].gradeLevelAssignments = [offset % 2 === 0 ? "Grade 11" : "Grade 12"];

    if (offset < shsExpertiseSubjects.length) {
      seeded[index].subjectExpertise = [shsExpertiseSubjects[offset]];
    } else {
      seeded[index].subjectExpertise = [];
    }
  });

  return seeded.map((member) => normalizeFacultyRecord(member, null));
}
...
function getCreatedSubjectCatalog() {
  if (typeof window === "undefined") {
    return [];
  }
...
function getSubjectOptionsFromCatalog(catalog, level) {
  const scoped = catalog.filter((subject) => String(subject.schoolLevel || "").toLowerCase() === level);
  const values = scoped
    .map((subject) => {
      const code = sanitizeText(subject.subjectCode);
      const name = sanitizeText(subject.subjectName);
...
function pickJhsCoverageSubjects(jhsSubjects) {
  const pickByKeyword = (keywords) =>
    jhsSubjects.find((subject) => keywords.some((keyword) => subject.toLowerCase().includes(keyword)));

  const candidates = [
    pickByKeyword(["english"]),
    pickByKeyword(["math"]),
    pickByKeyword(["filipino"]),
    pickByKeyword(["science"]),
    pickByKeyword(["araling", "panlipunan"]),
    pickByKeyword(["mapeh", "music", "arts", "health", "pe"]),
    pickByKeyword(["tle", "technology", "livelihood"]),
    pickByKeyword(["esp", "edukasyon", "values"]),
  ].filter(Boolean);
  ...
  return Array.from(new Set([...uniqueCandidates, ...jhsSubjects])).slice(0, Math.max(4, uniqueCandidates.length));
}
